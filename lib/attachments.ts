...
export function getFileKind(filename: string): "text" | "image" | null {
...
export async function readAttachedFile(file: File): Promise<AttachedFile | null> {
  ...
  return new Promise((resolve) => {
    ...
    reader.onload = (e) =>
      resolve({ name: file.name, mimeType: file.type, kind: "image", content: e.target?.result as string, size: file.size });
