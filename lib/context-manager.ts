...
export class ContextManager {
  ...
  private saveContext(): void {
    ...
    try {
      const serialized = {
        ...
        projectChatHistories: Array.from(
          this.workspaceContext.projectChatHistories.entries(),
        ),
      };
  ...
  updateProjectContext(project: ProjectContext): void {
    const existingIndex = this.workspaceContext.projects.findIndex(
      (p) => p.id === project.id,
    );
  ...
  addToProjectChatHistory(
    projectId: string,
    message: { role: string; content: string },
  ): void {
    const history =
      this.workspaceContext.projectChatHistories.get(projectId) || [];
    history.push({
      ...message,
      timestamp: new Date(),
    });
